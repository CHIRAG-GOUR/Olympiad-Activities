...
export function DivisibilityScannerActivity({
  value,
  onChange,
  readOnly = false }: DivisibilityScannerActivityProps) {
    ...
    <div className="bg-white border-2 border-slate-200 rounded-2xl p-4 text-slate-900 shadow-sm space-y-3.5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold text-lg text-cyan-700 flex items-center gap-2">
              4-Divisibility Scanner 
            </h3>
            <p className="text-xs text-slate-600">
              Rule: A number is divisible by 4 if and only if its <strong className="text-cyan-800">last two digits</strong> form a multiple of 4.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {options.map((opt) => (
          <div
            key={opt.id}
            className="p-3 bg-slate-50 border border-slate-200 rounded-xl flex flex-col items-center justify-center text-center gap-1 shadow-inner"
          >
            ...
            <div className="text-base font-black font-mono">
              ...
              <span className="text-cyan-800 bg-cyan-50 border border-cyan-200 px-1 py-0.5 rounded border border-cyan-500/40">
                {opt.last2}
              </span>
            </div>
