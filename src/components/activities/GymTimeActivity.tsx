...
export function GymTimeActivity({
  value,
  onChange,
  readOnly = false }: GymTimeActivityProps) {
  ...
  return (
    <div className="bg-white border-2 border-slate-200 rounded-2xl p-4 text-slate-900 shadow-sm space-y-3.5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-600">
              Calculate elapsed duration from <strong className="text-slate-900 font-mono">18:25</strong> to <strong className="text-slate-900 font-mono">19:52</strong>.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-around flex-wrap gap-4">
        <div className="p-3.5 bg-white border border-slate-200 rounded-xl text-center space-y-1">
          <span className="text-xs font-mono text-slate-600">Workout Start</span>
          <div className="font-black text-2xl text-amber-700 font-mono">18:25</div>
        ...
        <div className="p-3.5 bg-white border border-slate-200 rounded-xl text-center space-y-1">
          <span className="text-xs font-mono text-slate-600">Workout Finish</span>
          <div className="font-black text-2xl text-emerald-700 font-mono">19:52</div>
          <span className="text-[10px] text-slate-500 font-mono">7:52 PM</span>
        </div>

        <div className="text-2xl font-black text-amber-800">=</div>

        <div className="p-3.5 bg-amber-50 border border-amber-200 border-2 border-amber-400 rounded-xl text-center space-y-1 shadow-lg shadow-amber-500/20">
          <span className="text-xs font-mono text-amber-700 font-bold">Elapsed Duration</span>
          <div className="font-black text-2xl text-slate-900 font-mono">1 hr 27 min</div>
          <span className="text-[10px] text-amber-700 font-mono">87 total minutes</span>
        </div>
      </div>
        ...
        {options.map((opt) => {
          ...
          return (
            ...
            >
              ...
              {isSelected && <CheckCircle2 className="w-4 h-4 text-amber-800 shrink-0" />}
            </button>
          );
        })}
      </div>
