    ...
    <div className="bg-gradient-to-br from-slate-900 via-slate-950 to-slate-900 border border-slate-700/80 rounded-xl p-6 text-white shadow-2xl space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {options.map((opt) => (
          <div
            key={opt.id}
            className="p-3 bg-slate-950/90 border border-amber-500/30 rounded-xl flex flex-col items-center justify-center text-center gap-1 shadow-inner"
          >
            <span className="text-[10px] font-mono text-amber-400 font-bold">TABLET {opt.id}</span>
            <span className="font-mono text-sm font-black text-amber-200">{opt.expr}</span>
            <span className="text-xs font-mono text-slate-400 mt-1">= {opt.val}</span>
          </div>
        ))}
      </div>
