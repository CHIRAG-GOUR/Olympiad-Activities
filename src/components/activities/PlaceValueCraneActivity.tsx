    ...
    <div className="bg-gradient-to-br from-slate-900 via-slate-950 to-slate-900 border border-slate-700/80 rounded-xl p-6 text-white shadow-2xl space-y-6">
      ...
      <div className="space-y-2.5 max-w-xl mx-auto">
        {items.map((item, idx) => (
          ...
          >
            <div className="flex items-center gap-3">
              <span className="w-7 h-7 rounded-lg bg-slate-900 border border-slate-700 flex items-center justify-center font-mono font-bold text-sm text-yellow-400">
                {idx + 1}
              </span>
              <div>
                <h4 className="font-bold text-sm tracking-wide">{item.label}</h4>
                <span className="text-[11px] font-mono text-slate-400">Tier #{item.id}</span>
              </div>
            </div>
            ...
            {!readOnly && (
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  className="p-2 rounded bg-slate-900/80 hover:bg-slate-700 disabled:opacity-30 border border-slate-700 text-slate-300 transition"
                  title="Move Up"
                >
      ...
      <div className="p-3 bg-slate-950/80 border border-slate-800 rounded-lg text-center flex items-center justify-center gap-2 text-xs font-mono">
        {isAscendingCorrect ? (
