  ...
  return (
    <div className="space-y-6 animate-rise-in font-sans text-[#172033]">
      ...
      <div className="p-6 md:p-8 space-y-6 w-full max-w-[1750px] min-w-0">
        {subjects.length === 0 ? (
          <div className="bg-white border-2 border-[#E3E8EF] rounded-2xl p-12 text-center shadow-sm space-y-4 max-w-xl mx-auto">
            <div className="w-16 h-16 bg-[#F6F8FB] text-[#2563A8] rounded-2xl flex items-center justify-center mx-auto border border-[#E3E8EF]">
              <BookOpen className="w-8 h-8 text-[#2563A8]" />
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {subjects.map((sub) => (
              <div
                key={sub.id}
                className="bg-white border-2 border-[#E3E8EF] rounded-2xl p-6 shadow-sm flex flex-col justify-between space-y-5"
              >
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="font-mono font-bold text-[12px] bg-[#2563A8] text-white px-3 py-1 rounded-lg">
                      {sub.code}
                    </span>
                  ...
                  {/* Chapter taxonomy */}
                  <div className="pt-3 border-t border-[#E3E8EF] space-y-2.5">
                    <div className="text-[12px] font-bold uppercase tracking-wider text-[#1B4E88] flex items-center gap-2">
                      <FolderTree className="w-4 h-4 text-[#2563A8]" />
                      <span>Chapters & Topic Strands ({sub.chapters.length})</span>
                    </div>
                    <div className="space-y-2">
                      {sub.chapters.map((ch) => (
                        <div key={ch.id} className="p-3 bg-[#F6F8FB] rounded-xl border border-[#E3E8EF] text-xs">
                          ...
                          <div className="text-[11px] text-slate-600 mt-1.5 flex flex-wrap gap-1.5 font-bold">
                            {ch.topics.map((t) => (
                              <span key={t.id} className="bg-white px-2.5 py-0.5 rounded-md border border-[#E3E8EF] text-slate-800">
                                {t.name}
                              </span>
                            ))}
                          </div>
                ...
                <div className="pt-4 border-t border-[#E3E8EF] flex items-center justify-between text-[13px] font-bold font-mono">
                  <span className="text-[#2563A8]">{sub.questionCount || 0} Questions Linked</span>
